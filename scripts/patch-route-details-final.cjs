...
if (routeStart === -1 || routeEnd === -1) {
  throw new Error('Route panel section not found in src/App.tsx');
}
...
if (!app.includes('result-detail-grid')) {
  const anchorIndex = app.indexOf(explanationAnchor);
  if (anchorIndex === -1) {
    throw new Error('Explanation box not found in src/App.tsx');
  }
  ...
  app = app.slice(0, anchorIndex) + resultDetails + app.slice(anchorIndex + explanationAnchor.length);
}
...
console.log('Route stay details patch applied successfully.');
