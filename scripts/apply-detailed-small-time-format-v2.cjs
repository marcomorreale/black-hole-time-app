...
const start = source.indexOf('export function formatYears(value: number): string {');
if (start === -1) {
  throw new Error('formatYears function not found.');
}

const nextFunction = source.indexOf('\n\nexport function formatDistance', start);
if (nextFunction === -1) {
  throw new Error('formatDistance function not found after formatYears.');
}
...
source = source.slice(0, start) + newFunction + source.slice(nextFunction + 2);
fs.writeFileSync(filePath, source, 'utf8');
console.log('Detailed small time format v2 applied.');
