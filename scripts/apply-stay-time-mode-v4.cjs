...
write('src/logic/mission.ts', missionContent);

let app = read('src/App.tsx');
app = app.replace("import { useMemo, useState } from 'react';", "import { useEffect, useMemo, useState } from 'react';");
app = app.replace("import { calculateMission, getSuggestedRoutes } from './logic/mission';", "import { calculateMission, getSuggestedRoutes, RouteVisit, STAY_YEARS_PER_REAL_SECOND } from './logic/mission';");
...
app = app.replace("setRouteIds(route);", "setRouteVisits(route.map((stationId, index) => ({ stationId, arrivedAtMs: Date.now() + index * 1000 }))); ");
...
if (!app.includes('Scala permanenza')) {
  app = app.replace(
...
if (!app.includes('Eta finale gemello sulla Terra')) {
  app = app.replace(
...
console.log('Stay time mode v4 applied.');
